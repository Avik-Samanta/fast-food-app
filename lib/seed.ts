...
async function uploadImageToStorage(imageUrl: string) {
  try {
    ...
    const fileObj = {
      ...
      size: blob.size || 0,
      // Note: `react-native-appwrite` expects `uri` to be usable for RN uploads.
      // If this remote URL can't be uploaded from-device, we'll catch below and fall back.
      uri: imageUrl,
    };
...
async function seed(): Promise<void> {
  ...
  for (const item of data.menu) {
    try {
      ...
      if (!categoryId) {
        console.log(
          "❌ Missing category mapping for menu item:",
          item.name,
          "category:",
          item.category_name,
        );
      ...
      for (const cusName of item.customizations) {
        try {
          ...
          if (!customizationId) {
            console.log(
              "❌ Missing customization mapping for menu item:",
              item.name,
              "customization:",
              cusName,
            );
          ...
          console.log(
            "❌ Failed to create menu customization link:",
            "menuItem=",
            item.name,
            "customization=",
            cusName,
      ...
      console.log("❌ Failed to seed menu item:", item.name, error);
    }
  }
